...
import type { AddSaleItemInput, Customer, DiscountInput, Product, Sale, SaleItem } from '@/domain/models/pos';
import { usePosFlow } from '@/main/factories/flows/use-pos-flow';
import { useSearchProductsQuery } from '@/main/factories/queries/pos';
...
export function PosPage({ operatorName }: { operatorName: string | undefined }) {
  ...
  const handleCancelSale = useCallback(async () => {
    ...
    try {
      ...
      // apps/e2e/tests/support/pos.ts:78).
      focusScan();
    } catch (error) {
      toast(apiErrorMessage(error), 'danger');
    }
